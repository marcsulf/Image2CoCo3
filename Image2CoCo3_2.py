...
"""
ImagefileHead="\x00 \x00`\x00"
ImagefileFoot="\xff\x00\x00\x00\x00"
LImagefileMax = 8192
TailColor = 0
LTailData = 2048
...
***************************************************************************************************************
"""
filename=raw_input("""Please provide the filename of the image file.
Note: 6 characters or less for the base of the name is best: """)
while True:
    Choice = raw_input("Will you be displaying the picture on an (R)GB (recommended) or (C)MP monitor? ")
    if Choice[0].upper()=="R":
...
while not(ratio==CCRatio):
    Choice = raw_input("""The image is not the same aspect ratio as the screen. 
Do you want to strech the image to fill the screen? """)
    if Choice[0].upper()=="Y":
        ...
        Stretch = 0
        while True:
            Choice1 = raw_input("""Do you want to change the background color [default: black]? """)
        ...
        print("Invalid input!")
while True:
    Choice = raw_input("""Dithering (recommended) interleaves colors from the available palette to approximate other colors. 
...
for row in range(H): #go through the data one row at a time
    ...
    if bpp==8: #makes sure the BMP is actually stored as 8 bit color
        ...
        B=map(add,[i<<4 for i in A[::2]],A[1::2]) #Shift the bits in the even bytes by 4 places, then add that to the odd bytes to pack two pixels into one byte
        rowData = ''#clear the 1 Bpp row data
        rowData ="".join([struct.pack('B',i) for i in B]) #store the new 0.5 Bpp row data as a string
